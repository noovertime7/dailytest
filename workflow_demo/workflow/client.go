...
func (c *Client) SubmitCronTask(ctx context.Context, task Task, crontabexp string) (cron.EntryID, error) {
	log := log.FromContextOrDiscard(ctx).WithValues("task", task, "cron", crontabexp)
	log.Info("register cron task")
	cronID, err := c.crontab.AddFunc(crontabexp, func() {
...
func (c *Client) DisableCronTask(cronID cron.EntryID) {
	log.Info("disable cron task")
	c.crontab.Remove(cronID)
}
