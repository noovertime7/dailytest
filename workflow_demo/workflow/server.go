...
func (s *Server) Run(ctx context.Context) error {
	...
	return retry.OnKubeError(retry.NotContextCancelError, func() error {
		log.Println("starting work consumer...")
		if err := s.backend.Sub(ctx, "submit", s.consume, WithConcurrency(5), WithAutoACK(true)); err != nil {
			log.Println(err, "subscripe failed, retry...")
			return err
		}
		return nil
	})
...
func (n *Server) execute(ctx context.Context, task *jsonArgsStep) (err error) {
	...
	log.Println("executing", "step", task.Name, "func", task.Function, "args", task.Args)
	...
	defer func() {
		if e := recover(); e != nil {
			log.Println("executed panic", "step", task.Name, "func", task.Function, "err", e)
			switch e := e.(type) {
			default:
	...
	if e, ok := rvs[len(rvs)-1].Interface().(error); ok {
		err = e
		log.Println(err, "executed", "step", task.Name, "func", task.Function)
	} else {
		log.Println("executed", "step", task.Name, "func", task.Function, "result", task.Status.Result)
	}
	return err
}
