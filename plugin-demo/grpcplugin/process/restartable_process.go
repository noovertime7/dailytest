...
type RestartableProcess interface {
	AddReinitializer(key KindAndVersion, r Reinitializer)
	Reset() error
	ResetIfNeeded() error
	GetByKindAndVersion(key KindAndVersion) (interface{}, error)
	Stop()
}
...
type restartableProcess struct {
	...
	lock           sync.RWMutex
	process        Process
	plugins        map[KindAndVersion]interface{}
	reinitializers map[KindAndVersion]Reinitializer
	resetFailures  int
}
...
func newRestartableProcess(command string, logger logrus.FieldLogger, logLevel logrus.Level) (RestartableProcess, error) {
	p := &restartableProcess{
		...
		logger:         logger,
		logLevel:       logLevel,
		plugins:        make(map[KindAndVersion]interface{}),
		reinitializers: make(map[KindAndVersion]Reinitializer),
	}
...
func (p *restartableProcess) AddReinitializer(key KindAndVersion, r Reinitializer) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *restartableProcess) resetLH() error {
	...
	newPlugins := make(map[KindAndVersion]interface{})
...
// GetByKindAndVersion acquires the lock and calls getByKindAndVersionLH.
func (p *restartableProcess) GetByKindAndVersion(key KindAndVersion) (interface{}, error) {
	p.lock.Lock()
	defer p.lock.Unlock()

	return p.getByKindAndVersionLH(key)
}

// getByKindAndVersionLH returns the dispensed plugin for key. If the plugin hasn't been dispensed before, it dispenses a
// new one.
func (p *restartableProcess) getByKindAndVersionLH(key KindAndVersion) (interface{}, error) {
